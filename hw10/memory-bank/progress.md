...
- **AI agent infrastructure** (2026-05-17):
  - `.clinerules/coding-style.md` — coding style rules for C code
  - `.clinerules/project-guide.md` — project guide with context, boundaries, output requirements
...
- **Debounce for DISARM switch** (2026-05-17): Time-based debounce (50 ms) replaces tick-based counter in `rc_controls.c`. Constant `ARM_DEBOUNCE_MS 50`, uses `millis()` + `cmpTimeMs()`.
- **Blackbox refactor** (2026-05-18): Initialization logic extracted from `blackbox.c` into `blackbox_init.c/h`. 10 functions moved, `blackbox.c` reduced by ~60 lines. Build verified (STM32F405).
- **Unit tests for RX module** (2026-05-18): 13 unit tests for 4 functions in `rx.c` (`isPulseValid()`, `applyRxChannelRangeConfiguraton()`, `updateLinkQualitySamples()`, `resetAllRxChannelRangeConfigurations()`). All pass.
...
### AI Agent Tasks (Suggested Next)
- Task #1: ARM via sticks like DJI Mavic (not yet started)
- Code quality improvements based on `docs/rc_signals_analysis.md` (29 problems)
- Fixes for unresolved issues from `docs/unresolved-issues-report.md` (125 issues)
...
- **Debounce for DISARM switch**: Implementation pending ARM toolchain verification (code written, documented)
- **Unit tests**: 13 tests passing with gcc, clang not tested (potential differences in template/string handling)
...
- **AI agent infrastructure**: Standardized coding style rules, project guide, template tasks, and Definition of Done for AI-assisted development
- **Blackbox module refactored**: Cleaner separation of initialization logic from runtime logic
- **RX unit tests**: First step toward comprehensive test coverage of RC signal processing pipeline